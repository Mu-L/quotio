                ...
                if !matches!(doc.version, 1..=17)
                    ...
                    || (doc.version < 15
                        && (doc
                            .accounts
                            .iter()
                            .any(|account| account.keychain_account().is_some())
                            || doc.native_discovery.as_ref().is_some_and(|report| {
                                report
                                    .permissions
                                    .iter()
                                    .chain(&report.known_sources)
                                    .any(|source| source.keychain_account.is_some())
                            })))
                    ...
                    || (doc.version < 12
                        && doc
                            .resolved
                            .as_ref()
                            .is_some_and(|state| state.snapshot_digest.is_some()))
                    || (doc.version < 11
                        && doc
                            .resolved
                            .as_ref()
                            .is_some_and(|state| state.has_labels()))
...
impl Transaction {
    pub fn commit(mut self) -> Result<(), AccountError> {
        ...
        if !super::clients::valid(&self.document) {
