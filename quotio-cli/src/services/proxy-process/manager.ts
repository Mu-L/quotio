...
export async function startProxy(port = 8217): Promise<void> {
  ...
  if (!(await binaryFile.exists())) {
    throw new Error(`Proxy binary not found at ${binaryPath}. Run 'quotio proxy install' first.`);
  }
  ...
  if (currentProcess.exitCode !== null) {
    ...
    throw new Error(`Proxy failed to start (exit code: ${currentProcess.exitCode})`);
  }
