...
export class ManagementAPIClient {
  ...
  constructor(options: ManagementAPIClientOptions) {
    ...
    this.timeout = options.timeout ?? (this.isRemote ? DEFAULT_REMOTE_TIMEOUT : DEFAULT_LOCAL_TIMEOUT);
  }
  ...
  async deleteAuthFile(name: string): Promise<void> {
    await this.makeRequest(`/auth-files?name=${encodeURIComponent(name)}`, "DELETE");
  }
  ...
  async setRoutingStrategy(strategy: "round-robin" | "fill-first"): Promise<void> {
    try {
  ...
  async deleteAPIKey(key: string): Promise<void> {
    await this.makeRequest(`/api-keys?key=${encodeURIComponent(key)}`, "DELETE");
  }
  ...
  async setAuthFileDisabled(name: string, disabled: boolean): Promise<void> {
    ...
    await this.makeRequest(`${endpoint}?name=${encodeURIComponent(name)}`, "PUT");
  }
  ...
  async getProxyURL(): Promise<string> {
    const data = (await this.makeRequest("/proxy-url")) as { proxy_url: string };
...
export function createLocalClient(port: number = 8317, authKey: string = ""): ManagementAPIClient {
