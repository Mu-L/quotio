...
export ANTHROPIC_DEFAULT_HAIKU_MODEL="${haikuModel}"`;
    ...
    try {
      ...
      const mergedEnv = { ...(existingConfig.env as Record<string, string> ?? {}), ...quotioEnvConfig };
      ...
      if (mode === "automatic") {
        ...
        return {
          ...
          shellConfig: storageOption === "shell" || storageOption === "both" ? shellExports : undefined,
          rawConfigs,
          instructions,
          modelsConfigured: 3,
          backupPath,
  ...
  private generateCodexConfig(config: AgentConfiguration, mode: ConfigurationMode): AgentConfigResult {
    const home = homedir();
...
wire_api = "responses"`;
  ...
  private generateGeminiCLIConfig(config: AgentConfiguration, mode: ConfigurationMode): AgentConfigResult {
    const baseURL = config.proxyURL.replace(/\/v1$/, "");
...
export GEMINI_API_KEY="${config.apiKey}"`;
  ...
  private generateAmpConfig(config: AgentConfiguration, mode: ConfigurationMode): AgentConfigResult {
    const home = homedir();
...
export AMP_API_KEY="${config.apiKey}"`;
  ...
  private generateOpenCodeConfig(config: AgentConfiguration, mode: ConfigurationMode): AgentConfigResult {
    const home = homedir();
  ...
  private buildOpenCodeModelConfig(modelName: string): Record<string, unknown> {
    ...
    } else if (modelName.includes("codex") || modelName.startsWith("gpt-5") || modelName.startsWith("o1") || modelName.startsWith("o3")) {
      modelConfig.reasoning = true;
  ...
  private generateFactoryDroidConfig(config: AgentConfiguration, mode: ConfigurationMode): AgentConfigResult {
    const home = homedir();
