...
function getBinaryPaths(): string[] {
  ...
  if (process.platform === "darwin") {
    return ["/opt/homebrew/bin/cloudflared", "/usr/local/bin/cloudflared", "/usr/bin/cloudflared"];
  }
  return ["/usr/local/bin/cloudflared", "/usr/bin/cloudflared", "/snap/bin/cloudflared"];
}
...
function getVersion(binaryPath: string): string | null {
  try {
    const output = execSync(`"${binaryPath}" --version`, { encoding: "utf-8", timeout: 5000 });
...
class CloudflaredService {
  ...
  private state: TunnelState = { status: "idle", publicURL: null, errorMessage: null, startTime: null };
  ...
  async start(port: number): Promise<{ success: boolean; error?: string }> {
    ...
    if (!installation.isInstalled || !installation.path) {
      this.state = { status: "error", publicURL: null, errorMessage: "Cloudflared is not installed", startTime: null };
      return { success: false, error: "Cloudflared is not installed" };
    }

    const binaryPath = installation.path;
    this.state = { status: "starting", publicURL: null, errorMessage: null, startTime: null };
    this.outputBuffer = "";
    this.urlDetected = false;

    return new Promise((resolve) => {
      const args = ["tunnel", "--config", process.platform === "win32" ? "NUL" : "/dev/null", "--url", `http://localhost:${port}`];

      try {
        this.process = spawn(binaryPath, args, { stdio: ["ignore", "pipe", "pipe"] });
      } catch (err) {
        const error = err instanceof Error ? err.message : String(err);
        this.state = { status: "error", publicURL: null, errorMessage: error, startTime: null };
        this.cleanup();
      ...
      this.startTimeoutHandle = setTimeout(() => {
        if (this.state.status === "starting") {
          this.state = { status: "error", publicURL: null, errorMessage: "Tunnel start timed out", startTime: null };
          this.stop();
        }
      }, 30000);
      ...
      const handleOutput = (data: Buffer) => {
        ...
        if (match) {
          ...
          this.state = { status: "active", publicURL: match[0], errorMessage: null, startTime: new Date().toISOString() };
      ...
      this.process.on("error", (err) => {
        this.state = { status: "error", publicURL: null, errorMessage: err.message, startTime: null };
        this.cleanup();
      });

      this.process.on("exit", (code) => {
        if (this.state.status === "active" || this.state.status === "starting") {
          this.state = { status: "error", publicURL: null, errorMessage: `Tunnel exited unexpectedly (code: ${code})`, startTime: null };
        }
        this.cleanup();
      });
  ...
  async stop(): Promise<void> {
    this.clearStartTimeout();
    if (!this.process) {
      this.state = { status: "idle", publicURL: null, errorMessage: null, startTime: null };
      return;
    }
