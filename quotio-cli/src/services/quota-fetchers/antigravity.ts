...
const SUBSCRIPTION_API_URL = "https://cloudcode-pa.googleapis.com/v1internal:loadCodeAssist";
...
async function fetchSubscriptionInfo(accessToken: string): Promise<SubscriptionInfo | null> {
  try {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      for (const [name, info] of Object.entries(data.models ?? {})) {
        const isGeminiOrClaude = name.includes("gemini") || name.includes("claude");
...
export class AntigravityQuotaFetcher implements QuotaFetcher {
      ...
      try {
        ...
        if (isTokenExpired(content.expired) && content.refresh_token) {
          ...
          if (refreshed) {
            ...
            await persistRefreshedToken(filePath, refreshed.access_token, refreshed.expires_in);
          }
        }
        ...
        const effectiveTier = subscriptionInfo?.paidTier ?? subscriptionInfo?.currentTier;
        if (effectiveTier) {
  ...
  async fetchForAccount(account: string, accessToken: string): Promise<QuotaFetchResult> {
    try {
      const subscriptionInfo = await fetchSubscriptionInfo(accessToken);
      const projectId = subscriptionInfo?.cloudaicompanionProject;
      const quotaData = await fetchQuota(accessToken, projectId ?? undefined);
