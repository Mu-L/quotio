...
const STORAGE_JSON_PATH = "~/Library/Application Support/Trae/User/globalStorage/storage.json";
...
function isInstalled(): boolean {
  const appPaths = ["/Applications/Trae.app", expandPath("~/Applications/Trae.app")];
...
function convertToProviderQuota(
  ...
  if (quota?.premium_model_fast_request_limit && quota.premium_model_fast_request_limit > 0) {
  ...
  if (quota?.premium_model_slow_request_limit && quota.premium_model_slow_request_limit > 0) {
  ...
  if (quota?.advanced_model_request_limit && quota.advanced_model_request_limit > 0) {
...
export class TraeQuotaFetcher implements QuotaFetcher {
  ...
  async fetchAll(): Promise<QuotaFetchResult[]> {
    ...
    const entitlementResponse = await fetchEntitlementList(authData.accessToken, apiHost);

    let activeEntitlement: UserEntitlement | null = null;
    let resetTime: Date | null = null;

    const entitlementList = entitlementResponse?.user_entitlement_pack_list ?? [];
    ...
    const quotaData = convertToProviderQuota(authData, activeEntitlement, resetTime);
    const email = authData.email ?? authData.username ?? authData.userId ?? "Trae User";
  ...
  async fetchForAccount(
    ...
    const entitlementResponse = await fetchEntitlementList(accessToken, apiHost);
    ...
    const quotaData = convertToProviderQuota({ email: account }, activeEntitlement, resetTime);
