...
function convertToProviderQuota(entitlement: CopilotEntitlement): ProviderQuotaData {
  ...
  if (snapshots) {
    ...
    if (snapshots.premium_interactions && snapshots.premium_interactions.unlimited !== true) {
      models.push({
        name: "copilot-premium",
        percentage: calculateSnapshotPercent(snapshots.premium_interactions, 50),
        resetTime: resetTimeString,
      });
    }
  }
  ...
  if (models.length === 0) {
    ...
    if (remaining && total) {
      ...
      if (remaining.completions !== undefined && total.completions && total.completions > 0) {
...
export class CopilotQuotaFetcher implements QuotaFetcher {
  ...
  private async fetchFromAuthFile(file: LocalAuthFile): Promise<QuotaFetchResult | null> {
