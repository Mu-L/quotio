...
function isInstalled(): boolean {
  const appPaths = ["/Applications/Cursor.app", expandPath("~/Applications/Cursor.app")];
...
function readAuthFromStateDB(): CursorAuthData | null {
  ...
  try {
    ...
    try {
      const query = db.query("SELECT key, value FROM ItemTable WHERE key LIKE 'cursorAuth/%'");
...
function convertToProviderQuota(
  ...
  if (onDemand?.enabled) {
    ...
    if (onDemand.limit && onDemand.limit > 0 && onDemand.remaining !== undefined) {
  ...
  return {
    ...
    isForbidden: false,
    planType: formatPlanType(response?.membershipType ?? authData.membershipType),
  };
}
