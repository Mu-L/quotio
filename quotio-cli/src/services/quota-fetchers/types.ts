...
export interface QuotaFetcher {
  ...
  fetchForAccount(account: string, accessToken: string): Promise<QuotaFetchResult>;
}
...
export async function readAuthFiles(providerPrefix: string): Promise<LocalAuthFile[]> {
...
export function calculatePercentage(used: number | undefined, limit: number | undefined): number {
  if (limit === undefined || limit <= 0) return -1;
  if (used === undefined) return -1;
  return Math.min(100, Math.round((used / limit) * 100));
}

export function formatResetTime(resetAt: string | number | Date | undefined): string {
  if (!resetAt) return "";
