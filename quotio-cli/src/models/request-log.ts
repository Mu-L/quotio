...
export function createRequestLog(data: Omit<RequestLog, "id" | "timestamp">): RequestLog {
  return {
...
export function addEntryToStore(store: RequestHistoryStore, entry: RequestLog): RequestHistoryStore {
...
export function calculateStats(entries: RequestLog[]): RequestStats {
  ...
  const providerData: Record<string, { count: number; input: number; output: number; duration: number }> = {};
  const modelData: Record<string, { provider: string | null; count: number; input: number; output: number; duration: number }> = {};
  ...
  for (const entry of entries) {
    ...
    if (entry.provider) {
      const existing = providerData[entry.provider] ?? { count: 0, input: 0, output: 0, duration: 0 };
      existing.count++;
    ...
    if (entry.model) {
      const existing = modelData[entry.model] ?? { provider: entry.provider, count: 0, input: 0, output: 0, duration: 0 };
      existing.count++;
      existing.input += entry.inputTokens ?? 0;
      existing.output += entry.outputTokens ?? 0;
      existing.duration += entry.durationMs;
  ...
  for (const [key, data] of Object.entries(providerData)) {
    byProvider[key] = {
      ...
      averageDurationMs: data.count > 0 ? Math.round(data.duration / data.count) : 0,
    };
  }
