...
async function testAgentConnection(ctx: CLIContext, options: ConfigureOptions): Promise<CommandResult> {
  if (!options.agent) {
    logger.error("Agent is required. Use --agent or -a to specify.");
    logger.print(`\nAvailable agents: ${ALL_CLI_AGENTS.map((a) => a.id).join(", ")}`);
    return { success: false, message: "Agent is required" };
  ...
  try {
    ...
    if (!healthResponse.ok) {
      logger.print(`${colors.red("✗")} Proxy health check failed (HTTP ${healthResponse.status})`);
      return { success: false, message: `Proxy health check failed: HTTP ${healthResponse.status}` };
    }
  ...
  try {
    ...
    if (testResponse.status === 401) {
      logger.print(`${colors.yellow("!")} API key rejected - check your key or add one with: quotio auth login`);
      return { success: false, message: "API key rejected" };
    }

    if (testResponse.status === 503 || testResponse.status === 502) {
      logger.print(`${colors.yellow("!")} No active providers available - authenticate first: quotio auth login`);
      return { success: false, message: "No active providers" };
    }

    logger.print(`${colors.green("✓")} API endpoint is accessible (HTTP ${testResponse.status})`);
    ...
    return { success: true, data: { agent: agentId, proxyUrl, status: testResponse.status } };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
async function showAgentConfig(ctx: CLIContext, options: ConfigureOptions): Promise<CommandResult> {
  if (!options.agent) {
    logger.error("Agent is required. Use --agent or -a to specify.");
    logger.print(`\nAvailable agents: ${ALL_CLI_AGENTS.map((a) => a.id).join(", ")}`);
    return { success: false, message: "Agent is required" };
  ...
  if (status.installed) {
    ...
    logger.print(`  Configured: ${status.configured ? colors.green("Yes") : colors.yellow("No")}`);
  } else {
