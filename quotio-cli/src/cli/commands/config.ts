...
async function listConfig(client: ManagementAPIClient, ctx: CLIContext): Promise<CommandResult> {
  try {
    const config = await client.fetchConfig();
    ...
    if (ctx.format === "json") {
      ...
      logger.print(`  ${colors.cyan("routing")}:            ${config.routing?.strategy ?? "round-robin"}`);
      logger.print(`  ${colors.cyan("retry")}:              ${config.requestRetry ?? 0}`);
      logger.print(`  ${colors.cyan("max-retry-interval")}: ${config.maxRetryInterval ?? 0}s`);
      logger.print(`  ${colors.cyan("logging-to-file")}:    ${config.loggingToFile ?? false}`);

      try {
        const proxyUrl = await client.getProxyURL();
        logger.print(`  ${colors.cyan("proxy-url")}:          ${proxyUrl || colors.dim("(not set)")}`);
      } catch {
        logger.print(`  ${colors.cyan("proxy-url")}:          ${colors.dim("(not set)")}`);
      }
    }
  ...
  try {
    if (!key) {
      ...
      if (ctx.format === "json") {
        ...
        logger.print(`  max-retry-interval: ${DEFAULT_CONFIG["max-retry-interval"]}s`);
    ...
    const defaultValue = key === "proxy-url" ? "(cleared)" : DEFAULT_CONFIG[keyTyped];
